...
export default function Chat() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        ...
        // The backend now filters out messages from blocked users where the current user is the blocker
        // We only need to filter out messages where the current user is blocked by the receiver
        const filteredMessages = data.filter(msg => {
          // If this message is blocked and I am the sender, don't show it
          // (this means the receiver blocked me)
          if (msg.blocked && msg.is_sender) {
            return false;
          }
          return true;
        });
  ...
  useEffect(() => {
    const checkBlockStatus = async () => {
      try {
        const response = await fetch("/api/auth/blocks/", {
          headers: {
        ...
        if (response.ok) {
          ...
          const blocked = blocks.some(block => block.blocked == receiverId);
    ...
    try {
      ...
      const fetchMessages = async () => {
        ...
        try {
          ...
          const data = await response.json();
          const filteredMessages = data.filter(msg => !msg.blocked || msg.is_sender);
          setMessages(filteredMessages);
        } catch (err) {
          console.error(err);
        }
  ...
  useEffect(() => {
    const chatSocket = new WebSocket(
      `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}/ws/chat/${receiverId}/`
    );
    ...
    chatSocket.onmessage = function(e) {
      ...
      if (data.error) {
        ...
        setMessages(prevMessages => [...prevMessages, {
          ...
          sender_username: data.sender_username,
          timestamp: new Date().toISOString(),
          is_sender: data.sender_username === receiverName
        }]);
      }
    };

    const sendMessageViaSocket = (text) => {
    ...
    <div className="flex flex-col h-screen bg-gray-100">
          ...
          <div key={date}>
            ...
            {msgs.map((msg) => (
              <div
                ...
                } ${msg.error ? "border border-red-500" : ""} ${msg.warning ? "border border-yellow-500" : ""}`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex-grow">
                    ...
                    {msg.warning && (
                      <p className="text-xs text-yellow-300 mt-1">{msg.warning}</p>
                    )}
                  </div>
      ...
      {isBlocked ? (
        <div className="p-4 bg-red-100 text-red-800 text-center">
          <p>You have blocked this user. Unblock them from the Chat List page to resume messaging.</p>
        </div>
      ) : (
